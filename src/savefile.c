...
void write_save_helper(const char *savnam, FILE *wave)
{
    ...
    if (GLOBALS->loaded_file_name) {
        if ((GLOBALS->loaded_file_type == MISSING_FILE) || (GLOBALS->is_optimized_stdin_vcd)) {
            ...
            struct stat sbuf;
#endif
            char *unopt = GLOBALS->unoptimized_vcd_file_name ? GLOBALS->unoptimized_vcd_file_name
...
#endif
            if (GLOBALS->optimize_vcd && GLOBALS->unoptimized_vcd_file_name) {
                fprintf(wave, "[optimize_vcd]\n");
            }
            fprintf(wave,
                    "[savefile] \"%s\"\n",
    ...
    if (!GLOBALS->ignore_savefile_pane_pos) {
        ...
        if (GLOBALS->expanderwindow) {
            GLOBALS->sst_expanded =
                gtk_expander_get_expanded(GTK_EXPANDER(GLOBALS->expanderwindow));
    ...
    while (t) {
        if ((t->flags != def) || (t == GLOBALS->traces.first)) {
            ...
            if ((t->flags & TR_TTRANSLATED) && (!t->t_filter))
            ...
            if (t->vector &&
                ...
                for (ix = 0; ix < t->n.vec->bits->nnbits; ix++) {
                    if (nodes[ix]->expansion) {
                        fprintf(wave,
                                " (%d)%s",
                                nodes[ix]->expansion->parentbit,
                                append_array_row(nodes[ix]->expansion->parent));
                    } else {
                ...
                while (st) {
                    if (st->value == ST_STRING) {
                        fprintf(wave,
                                "?\"%s\n",
                                st->string ? st->string
                                           : ""); /* search type for this trace is string.. */
                    } else {
                        fprintf(wave,
                                "?%02x\n",
    ...
    defined HAVE_REALPATH || defined __MINGW32__
                                char *can =
                                    realpath_2(GLOBALS->ttranssel_filter[t->t_filter], NULL);
                                fprintf(wave, "^<%d %s\n", t->t_filter, can);
                                free(can);
#else
                                fprintf(wave,
                        ...
                        if (t->vector && !(t->n.vec->transaction_cache &&
                            ...
                            fprintf(wave,
                                    ...
                                    t->n.vec->transaction_cache
                                        ? t->n.vec->transaction_cache->bvname
                                        : t->n.vec->bvname);
    ...
    if (!wave) {
        ...
        if (extract_dumpfile_savefile_only) {
            while ((iline = fgetmalloc(wave))) {
                ...
                } else if (!strncmp(iline, "[dumpfile_mtime]", 16)) {
                    if (dumptim) {
                        struct tm tm;
                        time_t t;
                        ...
                        *dumptim = -1;
                        ...
                        if (lhq && rhq && (lhq != rhq)) {
                            ...
                            *rhq = 0;
                            slen = strlen(lhq + 1);
                            strp_buf = calloc_2(1, slen + 32); /* workaround: linux strptime seems
                                                                  to overshoot its buffer */
                ...
                } else if (!strncmp(iline, "[dumpfile_size]", 15)) {
                    if (dumpsiz) {
                        *dumpsiz = atoi_64(iline + 15);
                    }
                } else if (!strncmp(iline, "[savefile]", 10)) {
                    char *lhq = strchr(iline + 10, '"');
                    char *rhq = strrchr(iline + 10, '"');
                    if ((lhq) && (rhq) && (lhq != rhq)) /* no real need to check rhq != NULL*/
                    {
                ...
                } else if (!strncmp(iline, "[optimize_vcd]", 14)) {
                    if (opt_vcd) {
                        *opt_vcd = 1;
                    }
                }
...
int parsewavline(char *w, char *alias, int depth)
{
    ...
    if (*w2 == '*') {
        ...
        if ((GLOBALS->default_flags & (TR_FTRANSLATED | TR_PTRANSLATED)) ==
            (TR_FTRANSLATED | TR_PTRANSLATED)) {
            GLOBALS->default_flags &=
                ~TR_PTRANSLATED; /* safest bet though this is a cfg file error */
        }
        ...
        w2 = w2 + strlen(prefix);
        while (1) {
            if (isspace((int)(unsigned char)*w2)) {
                w2++;
                continue;
            }
            if (!(*w2))
        ...
        b = maketyp ? makevec(prefix + 1, w2)
                    : makevec_annotated(prefix + 1, w2); /* '#' vs ':' cases... */

        if (GLOBALS->default_flags & TR_GRP_BEGIN) {
            current_grp_depth = GLOBALS->group_depth;
        }
    ...
    grp_bot:
        ...
        if (*(w2 + 1) == '>') {
            ...
            if (*(w2 + 2) != '0') {
                /*			char *fn = strstr(w2+3, " "); */
                char *fn = w2 + 2;
                while (*fn && !isspace((int)(unsigned char)*fn))
                    fn++;
                if (fn) {
                    while (*fn && isspace((int)(unsigned char)*fn))
                        fn++;
        ...
        *w++ = 0;
        ...
        if (rc) {
            ...
            /* prevent malformed group openings [missing group opening] from keeping other signals
             * from displaying */
            ...
            if (sp) {
                *sp = 0;
                ...
                if (lbrack) {
                    ...
                    if (0) /* this is overkill for now with possible delay implications so commented
                              out */
                        if (!made) {
                            *lbrack = 0;
 ...
 * For example, if the current directory is C:\foo\bar and the filename C:\foo\whee\text.txt is
...
char *GetRelativeFilename(char *currentDirectory, char *absoluteFilename, int *dotdot_levels)
{
    ...
    *dotdot_levels = 0;
    ...
    if (cdLen > MAX_FILENAME_LEN || cdLen < ABSOLUTE_NAME_START + 1 || afLen > MAX_FILENAME_LEN ||
            ...
            if (grf) {
                ...
                if (rhs_this_save_slash) {
                    *(rhs_this_save_slash + 1) = 0;
                    ...
                    if (levels >
                        dotdot_levels) /* > because we left the ending slash on dup_this_save */
                    {
...
char *find_dumpfile(char *orig_save, char *orig_dump, char *this_save)
{
    ...
    if (!dfile && orig_save && orig_dump) {
        ...
        strcpy(orig_save2, pfx);
        strcat(orig_save2, orig_save);
        strcpy(orig_dump2, pfx);
        strcat(orig_dump2, orig_dump);
                    ...
                    if ((dumpsiz != -1) && (dumptim != -1)) {
                        ...
                        if (!stat(fni, &sbuf)) {
                            GLOBALS->dumpfile_is_modified =
                                (dumpsiz != sbuf.st_size) || (dumptim != sbuf.st_mtime);
                        }
                    }
#endif
                }
