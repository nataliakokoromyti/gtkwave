...
int main(int argc, char **argv)
{
...
#else
    gtk_widget_set_size_request(GTK_WIDGET(mainwindow),
                                400,
                                32); /* quartz doesn't retarget into mainwindow */
#endif
    ...
    hMapFile = CreateFileMapping(INVALID_HANDLE_VALUE,
                                 ...
                                 mapName);
    if (hMapFile != NULL) {
        dual_ctx = MapViewOfFile(hMapFile,
                                 FILE_MAP_ALL_ACCESS,
                                 0,
                                 0,
                                 2 * sizeof(struct gtkwave_dual_ipc_t));
        ...
        if (dual_ctx) {
            ...
            /* child 0 */
            {
                ...
                rc = CreateProcess(NULL, /* arglist[0] */
                                   mylist,
                                   NULL,
                                   NULL,
                                   FALSE,
            ...
            for (;;) {
                ...
                } else if ((dual_ctx[0].viewer_is_initialized) &&
                           (!dual_ctx[1].viewer_is_initialized)) {
            ...
            if (fork()) {
                if (fork()) {
                    ...
                    for (;;) {
                        ...
                        if ((!dual_ctx[0].viewer_is_initialized) &&
                            (dual_ctx[1].viewer_is_initialized)) {
                            gtk_window_set_title(GTK_WINDOW(mainwindow),
                                                 "TwinWave Waiting on Viewer #1");
                        } else if ((dual_ctx[0].viewer_is_initialized) &&
                                   (!dual_ctx[1].viewer_is_initialized)) {
                            gtk_window_set_title(GTK_WINDOW(mainwindow),
                                                 "TwinWave Waiting on Viewer #2");
                        } else if ((dual_ctx[0].viewer_is_initialized) &&
                                   (dual_ctx[1].viewer_is_initialized)) {
                            gtk_window_set_title(GTK_WINDOW(mainwindow), "TwinWave");
                            break;
